...
object Binary {
   ...
   */
  def apply(bytes: Byte*): Binary = unsafe_wrapArray(bytes.toArray)
   ...
   */
  def fromArray(array: Array[Byte]): Binary = fromArray(array, 0, array.length)
  ...
  def readFromInputStream(in: InputStream): Binary = {
    ...
    def read0(acc: Binary): Binary = {
      val lengthRead = in.read(buffer)
      if (lengthRead == -1) acc
      else read0(acc ++ Binary.fromArray(buffer, 0, lengthRead))
    }
    read0(Binary.empty)
  }
...
trait Binary extends RandomAccessSeq[Byte] with Binary.BinaryLike with Serializable {
  ...
  def splitAt(n: Int): (Binary, Binary) = (forcedSlice(0, n), forcedSlice(n, length))
  ...
  private def combine(left: Binary, right: Binary): Binary = {
    ...
    Binary.unsafe_wrapArray(combinedArray)
  }
   ...
   */
  final def unsafe_arrays(from: Int, until: Int): Iterable[ArrayBinary] = {
    if (from < 0 || until > length) throw new IndexOutOfBoundsException
   ...
   * certain optimisations.
   */
  final def unsafe_arrays: Iterable[ArrayBinary] =
